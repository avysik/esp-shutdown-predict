            ...
            "results": [
                {
                    ...
                    "telemetry_output_path": r.telemetry_output_path,
...
def run_batch(cfg: AppConfig) -> BatchSummary:
    ...
    def run_sequential() -> None:
        nonlocal ok, fail
        for run_id in tqdm(range(cfg.generation.n_files), total=cfg.generation.n_files, desc="Generating files"):
            try:
                res = worker_entry(cfg_dict, run_id, cfg.generation.n_files)
    ...
    if cfg.generation.workers == 1 or cfg.generation.n_files == 1:
        run_sequential()
    else:
        try:
            ...
            log.warning(
                "Process pool is unavailable in this environment (%s). Falling back to sequential execution.",
                exc,
            )
            run_sequential()
